...
const Home: React.FC = () => {
  const { messages, setMessages, isLoading, setIsLoading, chat } = useChat();
  ...
  const handleSendMessage = async (text: string) => {
    if (!chat.current) {
      ...
      const errorMessage: Message = {
        id: `error-${Date.now()}`,
        text: "Desculpe, o assistente não está pronto. Por favor, recarregue a página.",
        sender: 'bot',
        timestamp: new Date(),
      };
    ...
    try {
      const response = await chat.current.sendMessage({ message: text });
